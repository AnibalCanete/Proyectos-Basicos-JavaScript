...
const formulario = document.querySelector(".formulario-lorem");
const cantidad = document.getElementById("cantidad");
const resultado = document.querySelector(".texto-lorem");

formulario.addEventListener("submit", function (e) {
    /**
     * Un clic en el Botón de Envio de un Formulario Inicia su Envio al Servidor
     * A click on a form submit button - initiates its submision to the server
    */
    e.preventDefault();
    ...
    if (isNaN(valor) || valor < 0 || valor > 9) {
        resultado.innerHTML = `<p class="resultado">${texto[aleatorio]}</p>`;
    } else {
        let textoTemporal = texto.slice(0, valor);
        textoTemporal = textoTemporal.map(function (articulo) {
            return `<p class="resultado">${articulo}</p>`;
        }).join("");
